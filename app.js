/**
 * ==============================================================================
 * 📘 ESTUDIO DETALLADO LÍNEA POR LÍNEA: CRUD CON PERSISTENCIA EN LOCALSTORAGE
 * ==============================================================================
 ...
 * para crear un CRUD (Create, Read, Update, Delete) básico utilizando persistencia local.
...
// LÍNEA: const form = document.getElementById("formRegister");
// ------------------------------------------------------------------------------
// - ¿Qué es?: Una declaración de una constante ('const') llamada 'form'.
// - ¿Qué hace?: Almacena la referencia en memoria del formulario HTML con ID "formRegister".
// - ¿Cómo lo hace?: Busca en el HTML un elemento cuyo atributo id sea "formRegister" usando 'document.getElementById'.
// - ¿Por qué lo hace?: Para poder capturar el evento cuando el usuario presiona el botón de enviar (submit).
const form = document.getElementById("formRegister");

// ------------------------------------------------------------------------------
// 📌 SELECCIÓN DEL INPUT PARA EL NOMBRE
// LÍNEA: const nameInput = document.getElementById("nameInput");
// ------------------------------------------------------------------------------
// - ¿Qué es?: Una constante llamada 'nameInput'.
// - ¿Qué hace?: Almacena el campo de texto donde el usuario escribe su nombre en la página.
// - ¿Cómo lo hace?: Busca en el DOM el elemento con ID "nameInput".
// - ¿Por qué lo hace?: Para poder leer el texto que el usuario escribe (.value).
const nameInput = document.getElementById("nameInput");

// ------------------------------------------------------------------------------
// 📌 SELECCIÓN DEL INPUT PARA EL EMAIL
// LÍNEA: const emailInput = document.getElementById("emailInput");
// ------------------------------------------------------------------------------
// - ¿Qué es?: Una constante llamada 'emailInput'.
// - ¿Qué hace?: Almacena el campo de entrada del correo electrónico del usuario.
// - ¿Cómo lo hace?: Busca en el DOM el elemento con ID "emailInput".
// - ¿Por qué lo hace?: Permite extraer la dirección de correo ingresada por el usuario.
const emailInput = document.getElementById("emailInput");

// ------------------------------------------------------------------------------
// 📌 SELECCIÓN DEL CONTENEDOR DE LA TABLA
// LÍNEA: const tableBody = document.getElementById("tableBody");
// ------------------------------------------------------------------------------
// - ¿Qué es?: Una constante llamada 'tableBody'.
// - ¿Qué hace?: Almacena la referencia al cuerpo de la tabla HTML (<tbody>).
// - ¿Cómo lo hace?: Busca en el DOM el elemento con ID "tableBody".
// - ¿Por qué lo hace?: Es el lugar físico donde se insertarán las filas de datos dinámicamente.
const tableBody = document.getElementById("tableBody");

// ------------------------------------------------------------------------------
// 📌 SELECCIÓN DEL INPUT DE BÚSQUEDA
// LÍNEA: const searchInput = document.getElementById("searchInput");
// ------------------------------------------------------------------------------
// - ¿Qué es?: Una constante llamada 'searchInput'.
// - ¿Qué hace?: Almacena la referencia del cuadro de texto de búsqueda en la página.
// - ¿Cómo lo hace?: Busca en el DOM el elemento con ID "searchInput".
...
// ------------------------------------------------------------------------------
// 📌 CARGAR DATOS DESDE LOCALSTORAGE DE MANERA DETALLADA Y BÁSICA
// LÍNEA: let datosJSON = localStorage.getItem("DatoFormulario"); ...
// ------------------------------------------------------------------------------
...
// - ¿Cómo lo hace?: 
//     1. 'localStorage.getItem("DatoFormulario")': Obtiene los datos del disco como una cadena de texto (JSON) y los guarda en 'datosJSON'.
...
let datosJSON = localStorage.getItem("DatoFormulario");
let datos;
if (datosJSON === null) {
  datos = [];
} else {
...
// ==============================================================================
// 3. EVENT LISTENER: CONTROLADOR DEL FORMULARIO DE REGISTRO
// ==============================================================================

// ------------------------------------------------------------------------------
// 📌 ESCUCHAR CUANDO SE ENVÍA EL FORMULARIO (SUBMIT)
// LÍNEA: form.addEventListener("submit", function (eventoBotonEnviar) { ... });
// ------------------------------------------------------------------------------
...
form.addEventListener("submit", function (eventoBotonEnviar) {
  ...
  // LÍNEA: const name = nameInput.value;
  // ----------------------------------------------------------------------------
  // - ¿Qué es?: Una constante de texto que guarda el valor actual del campo de nombre.
  // - ¿Qué hace?: Guarda lo que el usuario escribió en el input de nombre.
  // - ¿Cómo lo hace?: Accede a la propiedad '.value' del input.
  // - ¿Por qué lo hace?: Permite almacenar temporalmente el texto ingresado para guardarlo en el objeto del usuario.
  const name = nameInput.value;

  // ----------------------------------------------------------------------------
  // 📌 OBTENER EL EMAIL ESCRITO POR EL USUARIO
  // LÍNEA: const email = emailInput.value;
  // ----------------------------------------------------------------------------
  // - ¿Qué es?: Una constante de texto que guarda el valor actual del campo de correo.
  // - ¿Qué hace?: Guarda lo que el usuario escribió en el input de correo.
  // - ¿Cómo lo hace?: Accede a la propiedad '.value' de 'emailInput'.
  ...
  if (name !== "" && email !== "") {
    ...
    // LÍNEA: const nuevoRegistro = { name: name, email: email };
    // --------------------------------------------------------------------------
    ...
    // - ¿Cómo lo hace?: Asigna el valor de la variable 'name' a la clave 'name', y el valor de la variable 'email' a la clave 'email'.
    ...
    // LÍNEA: salvarDatosEnLocalStorage();
    // --------------------------------------------------------------------------
    ...
    // LÍNEA: pintarTabla();
    // --------------------------------------------------------------------------
    // - ¿Qué es?: Una invocación a la función 'pintarTabla'.
    ...
    // - ¿Qué hace?: Borra y deja en blanco los inputs de texto del formulario automáticamente.
...
// ==============================================================================
// 3.5. EVENT LISTENER: CONTROLADOR DE BÚSQUEDA EN TIEMPO REAL
// ==============================================================================

// ------------------------------------------------------------------------------
// 📌 ESCUCHAR ENTRADAS DE TEXTO EN EL INPUT DE BUSCADOR
// LÍNEA: searchInput.addEventListener("input", function () { ... });
// ------------------------------------------------------------------------------
...
// - ¿Cómo lo hace?: Llama a la función 'pintarTabla()' cada vez que se detecta el evento de entrada `"input"`.
...
// ------------------------------------------------------------------------------
// 📌 DECLARACIÓN: FUNCIÓN PARA SALVAR DATOS EN MEMORIA LOCAL
// LÍNEA: function salvarDatosEnLocalStorage() { ... }
// ------------------------------------------------------------------------------
// - ¿Qué es?: Declaración de una función nominal llamada 'salvarDatosEnLocalStorage'.
...
function salvarDatosEnLocalStorage() {
  ...
  // ----------------------------------------------------------------------------
  // 📌 SERIALIZAR A JSON Y GUARDAR EN NAVEGADOR
  // LÍNEA: localStorage.setItem("DatoFormulario", JSON.stringify(datos));
  // ----------------------------------------------------------------------------
  // - ¿Qué es?: Invocación de la API de localStorage combinada con la conversión JSON.
  // - ¿Qué hace?: Convierte el arreglo 'datos' en texto plano y lo almacena con la clave "DatoFormulario".
  // - ¿Cómo lo hace?:
...
// LÍNEA: function pintarTabla() { ... }
// ------------------------------------------------------------------------------
// - ¿Qué es?: Declaración de la función nominal principal de renderizado 'pintarTabla'.
...
function pintarTabla() {
  ...
  for (let i = 0; i < datos.length; i++) {
    ...
    // - ¿Qué hace?: Compara si hay un texto de búsqueda escrito y si ni el nombre ni el email coinciden con él,
    //              salta al siguiente ciclo del bucle sin dibujar la fila.
    // - ¿Cómo lo hace?: 
    //     1. 'searchInput.value.toLowerCase()': Pasa a minúsculas el texto del buscador.
    //     2. 'elementoIndice.name.toLowerCase()': Pasa a minúsculas el nombre del registro.
    ...
    const nombreEnMinusculas = elementoIndice.name.toLowerCase();
    const emailEnMinusculas = elementoIndice.email.toLowerCase();

    if (terminoBusqueda !== "") {
      // Si el término de búsqueda NO coincide con el nombre Y tampoco coincide con el email...
      if (nombreEnMinusculas.includes(terminoBusqueda) === false && emailEnMinusculas.includes(terminoBusqueda) === false) {
    ...
    // --------------------------------------------------------------------------
    // 📌 ASOCIAR LA CLASE DE ESTILO CSS PARA EFECTO HOVER
    // LÍNEA: fila.classList.add("hover");
    // --------------------------------------------------------------------------
    // - ¿Qué es?: Adición de clase al elemento de fila.
    // - ¿Qué hace?: Añade la clase visual `"hover"` a la fila.
    ...
    // LÍNEA: const celdaName = document.createElement("td");
    // --------------------------------------------------------------------------
    ...
    // --------------------------------------------------------------------------
    // 📌 CREAR CELDA HTML PARA EL EMAIL
    // LÍNEA: const celdaEmail = document.createElement("td");
    // --------------------------------------------------------------------------
    // - ¿Qué es?: Creación de elemento de celda de tabla.
    // - ¿Qué hace?: Inicializa un objeto de tipo celda (<td>) para el correo.
    // - ¿Cómo lo hace?: Llama a 'document.createElement("td")'.
    // - ¿Por qué lo hace?: Contenedor para mostrar el email del usuario en la tabla.
    const celdaEmail = document.createElement("td");

    // --------------------------------------------------------------------------
    // 📌 CREAR CELDA HTML PARA LAS ACCIONES (BOTONES)
    // LÍNEA: const celdaAction = document.createElement("td");
    // --------------------------------------------------------------------------
    ...
    // LÍNEA: celdaAction.classList.add("table_body--tdActionCentrado");
    // --------------------------------------------------------------------------
    // - ¿Qué es?: Adición de clase CSS a la celda de acciones.
    // - ¿Qué hace?: Vincula la clase de estilos `"table_body--tdActionCentrado"`.
    ...
    // LÍNEA: botonEditar.classList.add("button", "button--Actualizargmail");
    // --------------------------------------------------------------------------
    // - ¿Qué es?: Adición de estilos CSS al botón de actualización.
    // - ¿Qué hace?: Agrega las clases `"button"` y `"button--Actualizargmail"` a este botón.
    ...
    // LÍNEA: botonEliminar.classList.add("button", "button--eliminar");
    // --------------------------------------------------------------------------
    // - ¿Qué es?: Adición de clases CSS al botón de eliminar.
    // - ¿Qué hace?: Agrega las clases `"button"` y `"button--eliminar"` a este botón.
    // - ¿Cómo lo hace?: Modifica las clases asociadas al botón en memoria.
    ...
    // - ¿Por qué lo hace?: Vincula de forma directa el botón de borrar de la fila con su registro en el array global.
    ...
    // LÍNEA: celdaName.textContent = elementoIndice.name;
    // --------------------------------------------------------------------------
    ...
    // LÍNEA: celdaEmail.textContent = elementoIndice.email;
    // --------------------------------------------------------------------------
    // - ¿Qué es?: Asignación a la propiedad 'textContent' de la celda.
    // - ¿Qué hace?: Escribe el valor del correo del registro actual en la celda de email.
    // - ¿Cómo lo hace?: Asigna el valor string '.email' al contenido textual de la celda.
    ...
    // LÍNEAS: celdaAction.appendChild(botonEditar); celdaAction.appendChild(botonEliminar);
    // --------------------------------------------------------------------------
    ...
    // LÍNEAS: fila.appendChild(celdaName); ...
    // --------------------------------------------------------------------------
    ...
    // LÍNEA: tableBody.appendChild(fila);
    // --------------------------------------------------------------------------
    ...
    // - ¿Cómo lo hace?: Inserta el elemento <tr> como un hijo del contenedor 'tableBody' en el DOM del navegador.
...
// - ¿Cómo lo hace?: Obtiene el objeto usando el índice, copia sus textos a los inputs y remueve el objeto del arreglo usando '.splice()'.
...
function editarDatos(indice) {
  ...
  // - ¿Cómo lo hace?: Accede al elemento del array mediante indexación por corchetes (`datos[indice]`).
  // - ¿Por qué lo hace?: Necesitamos acceder a sus atributos (.name y .email) para poder cargarlos en los campos de texto.
  const editarElementoIndice = datos[indice];

  // ----------------------------------------------------------------------------
  // 📌 CARGAR NOMBRE DEL REGISTRO EN EL FORMULARIO VISIBLE
  // LÍNEA: nameInput.value = editarElementoIndice.name;
  // ----------------------------------------------------------------------------
  // - ¿Qué es?: Asignación del nombre al campo de texto del formulario.
  // - ¿Qué hace?: Coloca el nombre original en el input de texto superior.
  // - ¿Cómo lo hace?: Copia el valor de la propiedad '.name' al valor visual del input.
  ...
  // LÍNEA: emailInput.value = editarElementoIndice.email;
  // ----------------------------------------------------------------------------
  // - ¿Qué es?: Asignación del correo al campo de correo del formulario.
  // - ¿Qué hace?: Coloca el correo original en el input de email superior.
  // - ¿Cómo lo hace?: Copia el valor de la propiedad '.email' al valor del input de email.
  ...
  // ----------------------------------------------------------------------------
  // 📌 GUARDAR LA LISTA EN EL STORAGE DEL NAVEGADOR
  // LÍNEA: salvarDatosEnLocalStorage();
  // ----------------------------------------------------------------------------
  ...
  // - ¿Cómo lo hace?: Llama a la función 'pintarTabla()'.
...
// - ¿Cómo lo hace?: Elimina el objeto del array con '.splice()', guarda la lista y actualiza la vista.
// - ¿Por qué lo hace?: Implementa la acción Delete (eliminar) básica del CRUD.
function eliminarDatos(indice) {

  // ----------------------------------------------------------------------------
  // 📌 BORRAR REGISTRO DE MEMORIA MEDIANTE INDICE
  // LÍNEA: datos.splice(indice, 1);
  // ----------------------------------------------------------------------------
...
// LÍNEA: pintarTabla();
// ------------------------------------------------------------------------------
// - ¿Qué es?: Ejecución síncrona de la función 'pintarTabla()' al terminar de procesar el archivo.
// - ¿Qué hace?: Dibuja todas las filas de usuarios que ya se encontraran guardadas en el LocalStorage al abrir la web.
// - ¿Cómo lo hace?: Llama a 'pintarTabla()' inmediatamente en el hilo principal del script.
